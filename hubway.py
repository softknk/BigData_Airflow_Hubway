...
clear_old_csv_folder = BashOperator(
    ...
    bash_command='rm -r /home/airflow/hubway/csv',
    dag=dag,
)
...
pyspark_create_kpi = SparkSubmitOperator(
    ...
    verbose=True,   
    application_args=['--hdfs_source_dir', '/user/hadoop/hubway'],
    dag=dag
)


create_local_import_dir >> clear_old_csv_folder >> clear_local_import_dir >> download_hubway_kaggle_data >> unzip_hubway_data >> create_hubway_csv_dir >> filter_and_copy_csv >> merge_csv_files >> create_hdfs_hubway_partition_dir >> hdfs_put_hubway_data >> pyspark_create_kpi
